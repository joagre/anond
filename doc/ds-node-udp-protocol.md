...
protocol message is encrypted within a 74 bytes cell. It provides a
number of services aimed at the member nodes of an anond overlay
network:
...
bytes is the actual protocol message and these can orginate from the
DS or from a node.
...
unique node id are generated by the DS when a node presents itself to
the DS, i.e. when it calls the `publish-node` method provided by the
[DS JSON-RPC/HTTPS server](ds-json-rpc-server.md).
...
DS NAT/firewall (if any). The nodes also reside behind NAT/firewalls
but there is no need to configure them to open up any UDP
ports. Instead
[UDP Hole punching techniques](http://en.wikipedia.org/wiki/UDP_hole_punching)
are used to make these ports available.
...
`publish-node` method and this informs the DS about the external
ip-address and UDP port which the node sits behind, i.e. a node
typically sits behind a NAT/firewall.
...
complete hop information on how a packet will travese the overlay
...
A node's routing table contains a route entry for each member node in
the overlay network, i.e. it can fairly large. For this reason the
routing table is sent in fragments (not larger than 480 bytes).

For this purpose a fragment counter and a fragment size are
incorporated in the protocol message. The last bit in the gragement
counter signals if this was the last fragment to be sent and the
fragment size is the size of the fragment in this protocol message.

If duplicated or reordered *ds-get-network-topology* protocol messages
arrives at the DS it is up to the DS to sort it out. If a
*ds-get-network-topology* dissapears in the network the fetching of
the node routing table will fail, i.e. there is no way to signal a
retransmit of a *ds-get-network-topology* protocol message fragment.
...
and as many neighbour nodes as "Number of Neighbours" indicates of these:
...
udp-port (16 bits, unsigned) | path-cost (16 bits, unsigned) | Flags (8 bits, unsigned),
