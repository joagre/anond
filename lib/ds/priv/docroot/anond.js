...
anond.neighbours = function(networkTopology, selectedNodes, handler) {
  ...
  for (var i = 0; i < networkTopology.length; i++) {
    if (networkTopology[i]["neighbour-nodes"].length == 0)
      continue;

    var source = anond._lookupNode(networkTopology[i].na, nodes);
    anond._assert(source != -1, "source must not be -1");

    for (var j = 0; j < networkTopology[i]["neighbour-nodes"].length; j++) {
      var link = anond._lookupLink(networkTopology[i].na,
                                   networkTopology[i]["neighbour-nodes"][j].na,
                                   links);
      if (link == null) {
        var target = anond._lookupNode(
          networkTopology[i]["neighbour-nodes"][j].na, nodes);
        ...
        var incomingNeighbourNode = false;
        if (selectedNodes.length != 0)
          if (selectedNodes.indexOf(networkTopology[i].na) != -1)
            incomingNeighbourNode =
              networkTopology[i]["neighbour-nodes"][j]
        ...
        links[k++] = {
          ...
          "to": networkTopology[i]["neighbour-nodes"][j].na,
          "path-cost": networkTopology[i]["neighbour-nodes"][j]["path-cost"],
          "visible": visible,
          "incoming-neighbour-node": incomingNeighbourNode
        };
      } else {
        link["path-cost"] =
          (link["path-cost"]+
           networkTopology[i]["neighbour-nodes"][j]["path-cost"])/2;
