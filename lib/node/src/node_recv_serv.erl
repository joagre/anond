...
loop(#serv_state{parent = Parent,
                 receiver = Receiver,
                 socket = Socket,
                 my_na = {MyNaIpAddress, MyNaPort}} = S) ->
    receive
        ...
        {handshake, {node_tun_serv, TunFd}} ->
            ...
            ok = gen_udp:send(Socket, MyNaIpAddress, MyNaPort,
                              <<?MESSAGE_ARRIVED:8>>),
            loop(S);
...
receiver(#receiver_state{node_db = NodeDb,
                         ...
                         tun_fd = TunFd,
                         socket = Socket,
                         my_na = {MyNaIpAddress, MyNaPort} = MyNa,
                         my_oa = MyOa} = S) ->
    case gen_udp:recv(Socket, 0) of
        {ok, {MyNaIpAddress, MyNaPort, <<?MESSAGE_ARRIVED:8>>}} ->
            receive
        ...
        {ok, {PeerIpAddress, PeerIpPort, Cell}} ->
            handle_cell(NodeDb, RouteDb, NodeRouteServ, NodePathCostServ,
                        TunFd, MyNa, MyOa, PeerIpAddress, PeerIpPort, Cell),
            receiver(S);
        {error, Reason} ->
            ?daemon_log("UDP failure on ~s:~w: ~s",
                        [net_tools:string_address(MyNaIpAddress), MyNaPort,
                         inet:format_error(Reason)]),
            receiver(S)
    end.

%% write ipv6 packet to tun device
handle_cell(NodeDb, RouteDb, NodeRouteServ, NodePathCostServ, TunFd, MyNa,
    ...
    case TunFd of
        undefined ->
            handle_cell(NodeDb, RouteDb, NodeRouteServ, NodePathCostServ,
                        TunFd, MyNa, MyOa, PeerIpAddress, PeerPort, Rest);
        _ ->
            case tuncer:write(TunFd, Ipv6Packet) of
                ok ->
                    handle_cell(NodeDb, RouteDb, NodeRouteServ,
                                NodePathCostServ, TunFd, MyNa, MyOa,
                                PeerIpAddress, PeerPort, Rest);
                {error, Reason} ->
...
handle_cell(NodeDb, RouteDb, NodeRouteServ, NodePathCostServ, TunFd, MyNa, MyOa,
            PeerIpAddress, PeerPort,
            <<?IP_PACKET:8,
    ...
    handle_cell(NodeDb, RouteDb, NodeRouteServ, NodePathCostServ, TunFd, MyNa,
                MyOa, PeerIpAddress, PeerPort, Rest);
...
handle_cell(NodeDb, RouteDb, NodeRouteServ, NodePathCostServ, TunFd, MyNa, MyOa,
    ...
    case decode_hops(Hops) of
        {ok, DecodedHops} ->
            ...
            handle_cell(NodeDb, RouteDb, NodeRouteServ, NodePathCostServ, TunFd,
                        MyNa, MyOa, PeerIpAddress, PeerPort, Rest);
        {error, Reason} ->
            ?dbg_log(Reason),
            handle_cell(NodeDb, RouteDb, NodeRouteServ, NodePathCostServ, TunFd,
                        MyNa, MyOa, PeerIpAddress, PeerPort, Rest)
...
handle_cell(NodeDb, RouteDb, NodeRouteServ, NodePathCostServ, TunFd,
    ...
    send({PeerIpAddress, PeerPort}, NodeDb, RouteDb, EchoReply),
    handle_cell(NodeDb, RouteDb, NodeRouteServ, NodePathCostServ, TunFd, MyNa,
                MyOa, PeerIpAddress, PeerPort, Rest);
...
handle_cell(NodeDb, RouteDb, NodeRouteServ, NodePathCostServ, TunFd, MyNa, MyOa,
    ...
    handle_cell(NodeDb, RouteDb, NodeRouteServ, NodePathCostServ, TunFd, MyNa,
                MyOa, PeerIpAddress, PeerPort, Rest);
%% throw away padding
handle_cell(_NodeDb, _RouteDb, _NodeRouteServ, _NodePathCostServ, _TunFd, _MyNa,
            _MyOa, _PeerIpAddress, _PeerPort, <<?PADDING:8, _/binary>>) ->
    ok;
%% no padding to throw away
handle_cell(_NodeDb, _RouteDb, _NodeRouteServ, _NodePathCostServ, _TunFd,
            _MyNa, _MyOa, _PeerIpAddress, _PeerPort, <<>>) ->
    ok;
%% unknown cell fragment
handle_cell(_NodeDb, _RouteDb, _NodeRouteServ, _NodePathCostServ, _TunFd, _MyNa,
            _MyOa, PeerIpAddress, PeerPort, Data) ->
    ?daemon_log("An unknown cell fragment received from ~s:~w: ~w",
                [net_tools:string_address(PeerIpAddress), PeerPort, Data]).
